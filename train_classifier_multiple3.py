...
class TransformerEncoder(keras.layers.Layer):
    def __init__(self, **kwargs):
        ...
        self.attention = keras.layers.MultiHeadAttention(
            num_heads=1, key_dim=1536, dropout=0.3
        )
        self.dense_proj = keras.Sequential(
            [keras.layers.Dense(3, activation=tf.nn.relu6), keras.layers.Dense(1536),]
        )
    ...
    def call(self, inputs, mask=None):
        ...
        proj_input = self.layernorm_1(inputs + inputs)
...
x = PositionalEmbedding()(input_classifier)
x = TransformerEncoder(name="transformer_layer")(x)
x = keras.layers.GlobalMaxPooling1D()(x)
x = keras.layers.Dropout(0.5)(x)
classifier = keras.layers.Dense(16, activation='sigmoid')(x)
...
classifier_model.save('test_transformer.h5')
'''
extracted_features = []

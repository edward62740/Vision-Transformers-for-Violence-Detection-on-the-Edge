    ...
    for i in range(0, len(test_images - 16)):
        ...
        if 1:
            ...
            if any(i < idx <= i + 16 for idx in test_img_new_idx):
                ...
                continue
            image = test_images[i].astype(np.int8)
            # Process the image through VIT model
            ...
            if len(feature_buffer) == 16:
                ...
                input_data = np.zeros((1, 1)).astype(np.int8)
                ...
                # acc
                if (cls_output[0][0] > 0) and test_labels[i] == 1:
                    running_acc += 1
                elif (cls_output[0][0] <= 0) and test_labels[i] == 0:
                    running_acc += 1

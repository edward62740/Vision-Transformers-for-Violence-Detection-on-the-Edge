...
from model import TemporalExtractor2, TemporalExtractor3, TemporalExtractor, TemporalExtractor4, TemporalExtractor5, \
    PosEncodingQuantizer, TransformerEncoderQuantizeConfig, CLSTokenLayer, PositionalEmbedding, TransformerEncoder, \
    TemporalExtractor4a
...
train_labels = np.load(PROJ_DIR + r'/output_features15_deit_final/train_lbl.npy', allow_pickle=True)
train_img_new_idx = np.load(PROJ_DIR + r'/output_features15_deit_final/train_images_new_idx.npy', allow_pickle=True)

test_labels = np.load(PROJ_DIR + r'/output_features15_deit_final/test_lbl.npy', allow_pickle=True)
test_img_new_idx = np.load(PROJ_DIR + r'/output_features15_deit_final/test_images_new_idx.npy', allow_pickle=True)

extracted_features = np.load(PROJ_DIR + r'/output_features15_deit_final/train.npy', allow_pickle=True)
extracted_features_test = np.load(PROJ_DIR + r'/output_features15_deit_final/test.npy', allow_pickle=True)

# output_features15_main_cnn2 works best
classifier_model = TemporalExtractor5()

classifier_model = tfmot.quantization.keras.quantize_annotate_model(classifier_model)

with tfmot.quantization.keras.quantize_scope(
        {'PosEncodingQuantizer': PosEncodingQuantizer,
         'PositionalEmbedding': PositionalEmbedding, },
        {'TransformerEncoderQuantizeConfig': TransformerEncoderQuantizeConfig,
         'TransformerEncoder': TransformerEncoder, },
{'TransformerEncoderQuantizeConfig': TransformerEncoderQuantizeConfig,
         'CLSTokenLayer': CLSTokenLayer, }
):
    # Use `quantize_apply` to actually make the model quantization aware.
    classifier_model = tfmot.quantization.keras.quantize_apply(classifier_model)

classifier_model.build(input_shape=[(None, 1, 192), (None, 16, 192)])  # 16?
optimizer2 = tf.keras.optimizers.RMSprop(learning_rate=1e-4)
optimizer = tf.keras.optimizers.SGD(learning_rate=3e-4)  # 1e-3
optimizer3 = tf.keras.optimizers.Adam(learning_rate=1e-3)  # 1e-3

classifier_model.compile(optimizer=optimizer, loss=keras.losses.BinaryCrossentropy(from_logits=True),
...
for epoch in range(num_epochs):
    ...
    for i in indices:
        ...
        buffer_array = np.concatenate(
            [np.expand_dims(extracted_features[i], axis=0) for i in range(start_idx, end_idx)], axis=0)
        ...
        loss, acc, p, r = classifier_model.train_on_batch([np.zeros((1, 1, 192)), buffer_array],
    ...
    for step in range(0, len(extracted_features_test) - 16, 16):
        ...
        buffer_array = np.concatenate(
            [np.expand_dims(extracted_features_test[i], axis=0) for i in range(start_idx, end_idx)], axis=0)
        buffer_array = np.expand_dims(buffer_array, axis=0)
        ...
        last_value = label_buffer_array[0, -1]
        out = classifier_model.predict([np.zeros((1, 1, 192)), buffer_array])
        test_output.append(out[0][0])
        test_used_labels.append(np.array([last_value]))

        loss, acc, p, r = classifier_model.test_on_batch([np.zeros((1, 1, 192)), buffer_array],
                                                         np.array([[last_value, not last_value]]))
        print(out)
        test_ctr += 1
